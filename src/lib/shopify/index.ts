...
const domain = env.SHOPIFY_STORE_DOMAIN
  ? ensureStartWith(env.SHOPIFY_STORE_DOMAIN, 'https://')
  : '';
...
const key = env.SHOPIFY_PUBLIC_ACCESS_KEY;
...
export async function getCollectionProducts({
  ...
  const response = await shopifyFetch<ShopifyCollectionProductsOperation>({
  ...
  return reshapeProducts(
    removeEdgesAndNodes(response.body.data.collection.products)
  );
}
...
export async function createCart(
  input: ShopifyCartCreateOperation['variables']['input']
): Promise<Cart | undefined> {
  const response = await shopifyFetch<ShopifyCartCreateOperation>({
...
export async function getCheckoutUrl(cartId: string): Promise<string> {
  const response = await shopifyFetch<ShopifyCartGetCheckoutUrlOperation>({
  ...
  return response.body.data.cart.checkoutUrl;
}
